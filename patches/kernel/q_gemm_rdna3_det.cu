...
// float -> T, per-dtype (mirrors tzero). The generic (T) cast is disabled for
// half under __HIP_NO_HALF_CONVERSIONS__, so the reduce kernel casts through
// this instead.
template <typename T>
__forceinline__ __device__ T fromfloat(float v) {
...
__global__ void gemm_q4_kernel_rdna3(const T* __restrict__ a,
                                     ...
                                     float* __restrict__ c, const int size_m,
  ...
  // Fixed-order accumulation: store this tile's fp32 partial sums into its own
  // K-slice of the scratch buffer `c`, laid out [num_k_splits, M, N] (linear:
  // z * M*N + m*N + n). Each element of a slice is written by exactly one tile
  // (one z per (m, n)), so no zero-fill is needed. A separate reduce kernel
  // then sums the slices in index order 0, 1, 2, ... and writes the activation
  // dtype once. This is bit-for-bit repeatable (unlike the atomicAdd order).
  #pragma unroll
  for (int m = 0; m < M_COUNT; ++m) {
    if (offset_m + m >= size_m) continue;  // skip padding rows past size_m
    float* out = c + blockIdx.z * ((size_t)size_m * size_n) +
                 (size_t)(offset_m + m) * size_n + n;
    out[0] = block_c[m][0];
    out[1] = block_c[m][1];
    out[2] = block_c[m][2];
    out[3] = block_c[m][3];
...
// ---------------------------------------------------------------------------
// Fixed-order reduction over the K-splits. For each output element (m, n) it
// sums scratch[z][m][n] for z = 0, 1, 2, ... in a fixed order and writes the
// result, cast to the activation dtype T, once. One thread per (m, n) element.
// ---------------------------------------------------------------------------
template <typename T>
__global__ void reduce_k_splits_rdna3(const float* __restrict__ scratch,
                                      T* __restrict__ out, int size_m,
                                      int size_n, int num_k_splits) {
  const int n = blockIdx.x * blockDim.x + threadIdx.x;
  const int m = blockIdx.y;
  if (n >= size_n) return;
  float sum = 0.0f;
  const float* p = scratch + (size_t)m * size_n + n;
  for (int z = 0; z < num_k_splits; ++z) {
    sum += p[(size_t)z * size_m * size_n];
  }
  out[(size_t)m * size_n + n] = fromfloat<T>(sum);
}
...
__global__ void gemm_q4_kernel_rdna3(const T*, const uint32_t*, const uint32_t*,
                                     const T*, float*, const int, const int,
                                     const int, const int, const int) {}

template <typename T>
__global__ void reduce_k_splits_rdna3(const float*, T*, const int, const int,
                                      const int) {}
...
void launch_gemm_q4_for_mcount(const T* a, const uint32_t* b_q_weight,
                               ...
                               float* c, T* out, int size_m, int size_n,
  ...
  // Fixed-order reduction over the gridDim.z K-splits. One thread per (m, n)
  // output element; grid.y covers size_m, grid.x covers size_n.
  const int num_k_splits = (int)grid.z;
  dim3 rblock(256);
  dim3 rgrid((size_n + 255) / 256, size_m);
  reduce_k_splits_rdna3<T><<<rgrid, rblock, 0, stream>>>(c, out, size_m, size_n,
                                                         num_k_splits);
}
...
void launch_gemm_q4(const T* a, const uint32_t* b_q_weight,
                    const uint32_t* b_qzeros, const T* b_scales, float* c,
                    T* out, int size_m, int size_n, int size_k, int groups,
                    bool use_v2_format, cudaStream_t stream) {
  const int zero_offset = use_v2_format ? 0 : 1;

  if (size_m == 1) {
    launch_gemm_q4_for_mcount<T, 1>(a, b_q_weight, b_qzeros, b_scales, c, out,
                                    size_m, size_n, size_k, groups, zero_offset,
                                    stream);
  } else if (size_m <= 3) {
    launch_gemm_q4_for_mcount<T, 2>(a, b_q_weight, b_qzeros, b_scales, c, out,
                                    size_m, size_n, size_k, groups, zero_offset,
                                    stream);
  } else if (size_m <= 7) {
    launch_gemm_q4_for_mcount<T, 4>(a, b_q_weight, b_qzeros, b_scales, c, out,
                                    size_m, size_n, size_k, groups, zero_offset,
                                    stream);
  } else {
    ...
    launch_gemm_q4_for_mcount<T, 8>(a, b_q_weight, b_qzeros, b_scales, c, out,
...
torch::Tensor gptq_gemm_rdna3(torch::Tensor a, torch::Tensor b_q_weight,
  ...
  // Fixed-order accumulation. Two buffers:
  //   scratch  [num_k_splits, M, N]  fp32 — each K-split tile stores its fp32
  //            partial sums into its own slice (no zero-fill, no atomics).
  //   out      [M, N]               activation dtype — the reduce kernel sums
  //            the slices in index order 0, 1, 2, ... and writes it once.
  const int num_k_splits = (size_k + BLOCK_KN_SIZE - 1) / BLOCK_KN_SIZE;
  at::Tensor scratch = torch::empty(
      {num_k_splits, size_m, size_n},
  ...
  if (a.scalar_type() == torch::kHalf) {
    vllm::gptq_rdna3::launch_gemm_q4<half>(
        ...
        (float*)scratch.data_ptr(), (half*)out.data_ptr(), size_m, size_n,
    ...
    vllm::gptq_rdna3::launch_gemm_q4<vllm::gptq_rdna3::bf16_t>(
        ...
        (float*)scratch.data_ptr(), (vllm::gptq_rdna3::bf16_t*)out.data_ptr(),
        size_m, size_n, size_k, groups, use_v2_format, stream);
  }

  // The reduce kernel already wrote the activation dtype; return it directly.
  return out;
}
